...
function load(filename, displayName) {
    ...
    xhr.open("GET", url, true);getStat
    xhr.send();

    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
            cur_book = xhr.responseText;

            getStat(cur_book);

            //remove line breaks and carriage returns and replace with a <br>
            cur_book = cur_book.replace(/(?:\r\n|\r|\n)/g, '<br>');

            document.getElementById("fileContent").innerHTML = cur_book;
